...
if __name__ == "__main__":
    ...
    try:
        if tiregroup != 'TBR' and BeadringType=="Tubeless": 
            try: 
                ...
                InitialK_Factor = float(lstSnsInfo["VirtualTireParameters"]["MoldKFactor"])
        else: 
            InitialK_Factor = float(lstSnsInfo["VirtualTireParameters"]["MoldKFactor"])
    except:
        pass 
    ...
    TIRE.plot_DetailLayout(filename_base+"-Deformed.png", Element, DeformedTopNode, dpi=300,\
         AddingNodes=SWPointForK_factor, rim=pRim, group=tiregroup, beadring=BeadringType, writeProfile=writeProfileNode)  ## if tiregroup != 'TBR': rimHt = 9.2    elif BeadringType=="Tubeless": 
